...
def upload_gen_uniq_filename(user_id):
    ...
    filename_uniq = str(user_id)+ "_" + time.strftime("%Y%m%d%H%M%S")
...
def upload_run_ai_and_update_db(uid, title, filepath):
    ...
    # Save image to S3
    s3 = boto3.client('s3')
    s3.upload_file(filepath, app.config['S3_BUCKET_ID'], title)
    ...
    query = '''INSERT INTO Images
               ...
               Values (%s, %s, %s, %s, %s, %s)
            '''
    cursor.execute(query,(uid, title, app.config['S3_BUCKET_ID'], num_faces, num_masks, num_nomasks))
    cnx.commit()
